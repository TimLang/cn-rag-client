...
#
#  $Revision: 8409 $
#  $Id: Globals.pm 8409 2013-02-16 17:02:11Z keplerbr $
#
#########################################################################
##
...
our %EXPORT_TAGS = (
	...
	state   => [qw($accountID $cardMergeIndex @cardMergeItemsID $charID @chars @chars_old %cart $field %homunculus $itemsList @itemsID %items $monstersList @monstersID %monsters @npcsID %npcs $npcsList @playersID %players @portalsID @portalsID_old %portals %portals_old $portalsList @storeList @skillsID %storage @storageID $storageTitle @arrowCraftID @spellsID %spells @unknownPlayers @unknownNPCs $useArrowCraft %currentDeal %incomingDeal %outgoingDeal @identifyID @partyUsersID %incomingParty @petsID %pets @buyerItemList $buyerID $buyingStoreID @buyerListsID @articles $articles %buyerLists %monsters_old @monstersID_old %npcs_old %items_old %players_old @playersID_old @servers $sessionID $sessionID2 $accountSex $accountSex2 $map_ip $map_port $KoreStartTime $secureLoginKey $initSync $lastConfChangeTime $petsList $playersList $portalsList @playerNameCacheIDs %playerNameCache %pet $pvp @cashList $slavesList @slavesID %slaves)],
	network => [qw($remote_socket $net $messageSender $charServer $conState $conState_tries $encryptVal $ipc $masterServer $lastSwitch $packetParser $clientPacketHandler $bytesSent $incomingMessages $outgoingClientMessages $enc_val1 $enc_val2 $status1 $status2)],
