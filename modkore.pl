...
#avoiding players
		foreach (@playersID) {
			...
			if (($ppl_control{$players{$_}{'name'}}{'teleport_auto'} || (($players{$_}{'name'} =~/^GM/ || $players{$_}{'name'} =~/^สายตรวจ/ || $GameMasters{$_}) && $config{'AvoidGM'}))
			...
			if (($ppl_control{$players{$_}{'name'}}{'disconnect_auto'} || (($players{$_}{'name'} =~/^GM/ || $players{$_}{'name'} =~/^สายตรวจ/ || $GameMasters{$_}) && $config{'AvoidGM'}==3))
				&& !$cities_lut{$field{'name'}.'.rsw'}) {
